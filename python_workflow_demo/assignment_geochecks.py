...
def value_validation(df):
    """
    Validate the Tijdstip and waardes columns in the DataFrame.
    ...
    def validate_tijdstip(tijdstip):
        # Check if Tijdstip is in a valid ISO 8601 format
        try:
            pd.to_datetime(tijdstip, format="%Y-%m-%dT%H:%M:%S.%f%z")
            return True
        except (ValueError, TypeError):
            return False

    def validate_waardes(waarde):
        # Check if waardes is a valid numeric value
        return isinstance(waarde, (int, float)) and not pd.isna(waarde)

    # Add validation columns
    df["Tijdstip_valid"] = df["Tijdstip"].apply(validate_tijdstip)
    df["waardes_valid"] = df["waardes"].apply(validate_waardes)
